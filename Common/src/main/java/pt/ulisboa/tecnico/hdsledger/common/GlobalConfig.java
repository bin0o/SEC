...
public class GlobalConfig {
  ...
  public static GlobalConfig fromFile(String path, String id) {
    try (BufferedInputStream is = new BufferedInputStream(new FileInputStream(path))) {
      ...
      config.setNodesMap(
          Arrays.stream(config.getNodesConfigs())
              .collect(Collectors.toMap(ProcessConfig::getId, e -> e)));
      ...
      Arrays.stream(config.getNodesConfigs())
          .forEach(
              processConfig -> {
                if (!processConfig.isClient()) servers.add(processConfig);

                File file =
                    new File(config.getKeysLocation() + "/public" + processConfig.getId() + ".key");
                try {
      ...
      // Parse tests
      config.setParsedTests(
          Arrays.stream(config.getTestsConfigs())
              .map(
                  test -> {
                    if (test.length > 0) {
                      return Arrays.stream(test)
                          .collect(Collectors.toMap(TestConfig::getId, e -> e));
                    } else {
                      return null;
                    }
                  })
      ...
      LOGGER.log(
          ...
          MessageFormat.format(
              "[GlobalConfig Tests]: Parsed tests for {0} instances",
              config.getParsedTests().size()));
  ...
  private Optional<State> getTestState(Integer instance) {
    ...
    if (index >= this.parsedTests.size()) return Optional.empty();

    Map<String, TestConfig> testConfig = this.parsedTests.get(index);

    if (testConfig == null) return Optional.empty();

    if (!testConfig.containsKey(currentNodeId)) return Optional.empty();
  ...
  public boolean dropMessage(Integer instance, MessageType type) {
    ...
    if (drop) {
      LOGGER.log(
          Level.WARNING,
          MessageFormat.format(
              "[GlobalConfig Tests]: DROP Message -> {0} / Instance {1}",
              type.toString(), instance));
    }
  ...
  public String tamperMessage(
    ...
    LOGGER.log(
        Level.INFO,
        MessageFormat.format(
            "[GlobalConfig]: Tampered {0} for instance {1}: {2}",
            type.toString(), instance, tamperedString));
