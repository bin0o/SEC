...
public class Link {
  ...
  public Link(GlobalConfig config, Class<? extends Message> messageClass) {
    this(
        config.getCurrentNodeConfig(),
        config.getCurrentNodeConfig().getPort(),
        config.getNodesConfigs(),
        messageClass,
        false,
        200);
  ...
  public void broadcast(Message data) {
    ...
    nodes.forEach(
        (destId, dest) -> {
          ...
          if (!dest.isClient()) send(destId, gson.fromJson(gson.toJson(data), data.getClass()));
        });
  }
    ...
    new Thread(
            () -> {
              try {
                ...
                if (node == null) throw new HDSSException(ErrorMessage.NoSuchNode);
                ...
                if (nodeId.equals(this.config.getId())) {
                  ...
                  LOGGER.log(
                      ...
                      MessageFormat.format(
                          "{0} - Message {1} (locally) sent to {2}:{3} successfully",
                ...
                for (; ; ) {
                  ...
                  if (receivedAcks.contains(messageId)) break;
                ...
                LOGGER.log(
                    Level.INFO,
                    MessageFormat.format(
                        "{0} - Message {1} sent to {2}:{3} successfully",
                        config.getId(), data.getType(), destAddress, destPort));
  ...
  public Message receive() throws IOException, ClassNotFoundException {
    ...
    if (!nodes.containsKey(senderId)) throw new HDSSException(ErrorMessage.NoSuchNode);
    ...
    switch (message.getType()) {
      ...
      case IGNORE -> {
        if (!originalType.equals(MessageType.COMMIT)) return message;
      }
      case PREPARE -> {
